...
<section class="service">
    <div class="conteiner">
        <h2 class="sub-title"> Добавление классификации </h2>
        <?php
            if (isset($_POST["name_class"])) {
                
                try {
                   
                    $sql = "INSERT INTO classification (name_class) VALUES (:name_class)";
                    // определяем prepared statement
                    $stmt = $dbh->prepare($sql);
                    // привязываем параметры к значениям
                    $stmt->bindValue(":name_class", $_POST["name_class"]);
                    // выполняем prepared statement
                    $affectedRowsNumber = $stmt->execute();
                    // если добавлена как минимум одна строка
                    if($affectedRowsNumber > 0 ){
                        header('Refresh: 2; url=classifications.php');  
                    }
                }
            ...
            <form method="post">
                <p>Название:
                <input type="text" name="name_class" /></p>
