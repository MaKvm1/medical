...
$stmt = $dbh->prepare("SELECT * FROM `work_schedule` JOIN cabinet ON work_schedule.id_cabinet=cabinet.id_cabinet JOIN users ON work_schedule.id_worker=users.id WHERE id_post = 2");
$stmt->execute();
...
<section class="service">
    <div class="conteiner">
        <h2 class="sub-title"> Управление расписанием </h2>
        <a type="button" href="createSchedule.php"> Добавить </a>
        <table class="table">
            <thead>
                <tr>
                    <th scope="col">#</th>
                    ...
                    <th scope="col">Окончание приема</th>
            ...
            <?php  foreach ($array as $key=>$row ): ?>
                <tr>
                    ...
                    <td><?= $row['full_name'] ?></td>
                    <td> + </td>
                    <td><button onclick="del(<?= $row['id_work'] ?>)" type="button" class="btn btn-outline-danger btn-sm">X</button></td>
                </tr>
            <?php endforeach; ?>
            </tbody>
        </table>
...
<script>
function del(id_work)
{
    $.ajax({
        url: 'DelClass.php',         /* Куда пойдет запрос */
        method: 'get',             /* Метод передачи (post или get) */
        dataType: 'html',          /* Тип данных в ответе (xml, json, script, html). */
        data: {id_work: id_work},     /* Параметры передаваемые в запросе. */
