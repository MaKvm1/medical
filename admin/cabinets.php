...
<section class="service">
    <div class="conteiner">
        <h2 class="sub-title"> Управление кабинетами </h2>
        <a type="button" href="createCabinet.php"> Добавить </a>
        <table class="table">
            <thead>
                <tr>
                    <th scope="col">#</th>
            ...
            <?php  foreach ($array as $key=>$row ): ?>
                <tr>
                    ...
                    <td><?= $row['name'] ?></td>
                    <td> + </td>
                    <td><button onclick="del(<?= $row['id_cabinet'] ?>)" type="button" class="btn btn-outline-danger btn-sm">X</button></td>
                </tr>
            <?php endforeach; ?>
            </tbody>
        </table>
...
<script>
function del(id_cabinet)
{
    $.ajax({
        url: 'DelCabinet.php',         /* Куда пойдет запрос */
        method: 'get',             /* Метод передачи (post или get) */
        dataType: 'html',          /* Тип данных в ответе (xml, json, script, html). */
        data: {id_cabinet: id_cabinet},     /* Параметры передаваемые в запросе. */
