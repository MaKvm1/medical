...
<section class="service">
    <div class="conteiner">
        <h2 class="sub-title"> Управление классификациями </h2>
        <a type="button" href="createClass.php"> Добавить </a>
        <table class="table">
            <thead>
                <tr>
                    <th scope="col">#</th>
            ...
            <?php  foreach ($array as $key=>$row ): ?>
                <tr>
                    ...
                    <td> + </td>
                    <td><button onclick="del(<?= $row['id_class'] ?>)" type="button" class="btn btn-outline-danger btn-sm">X</button></td>
                </tr>
            <?php endforeach; ?>
            </tbody>
        </table>
...
<script>
function del(id_class)
{
    $.ajax({
        url: 'DelClass.php',         /* Куда пойдет запрос */
        method: 'get',             /* Метод передачи (post или get) */
        dataType: 'html',          /* Тип данных в ответе (xml, json, script, html). */
        data: {id_class: id_class},     /* Параметры передаваемые в запросе. */
