...
<section class="service">
    <div class="conteiner">
        <h2 class="sub-title"> Добавление услуги </h2>
        <?php
            if (isset($_POST["name_serv"]) AND isset($_POST["id_classification"]) AND isset($_POST["price"])) {
                
                try {
                   
                    $sql = "INSERT INTO services (name_serv, id_classification, price) VALUES (:name_serv, :id_classification, :price)";
                    // определяем prepared statement
                    $stmt = $dbh->prepare($sql);
                    // привязываем параметры к значениям
                    $stmt->bindValue(":name_serv", $_POST["name_serv"]);
                    $stmt->bindValue(":id_classification", $_POST["id_classification"]);
            ...
            <form method="post">
                <p>Название:
                <input type="text" name="name_serv" /></p>
                <label for="exampleInputEmail1">Классификация</label>
                <select class="form-select" name="id_classification">
                    <option> Выберите классификацию </option>
                    <?php foreach ($array as $row): ?>
                    <option value=<?php echo $row['id_class']?>><?php echo $row['name_class']?></option>
                    <?php endforeach; ?>
                </select>
                <p>Стоимость:
